		...
		if (fEntitys[i]->hasComponents("Gravity"))
		{
			...
			for (int k = 0; k < (int)groundRays.size(); k++)
			{
				...
				if (t == -1)
				{
					...
					//if (fEntitys[i]->mPhysicsEntity->GetStates().isActiveProjectile && fEntitys[i]->getEntityId() == 1)
					//{
					//	std::cout << "X: ";
					//	std::cout << fEntitys[i]->;
					//	std::cout << " Y: " ;
					...
					fEntitys[i]->mPhysicsEntity->GetStates().SetIsJumping(false);
					fEntitys[i]->mPhysicsEntity->GetStates().SetIsActiveProjectile(false);
