...
static uint32_t util_hash32_str(const char *data)
{
	return util_hash32((const void *)data, (unsigned int)strlen(data));
}
...
ResourceCache::ResourceCache(const std::string &resourceDir)
{
	...
	for (int i = 0; i < SOUNDENGINE_BUFFER_POOL_SIZE; i++) {
		...
		m_cacheEntries[i].buffer = m_buffers[i];

		m_cacheEntryLoading[i] = false;
	}
}
...
ResourceCache::~ResourceCache()
{
	...
	alDeleteBuffers(SOUNDENGINE_BUFFER_POOL_SIZE, m_buffers);
}
...
uint32_t ResourceCache::PrefetchResource(const std::string &name)
{
	...
	if (!m_resources.count(hash) && !m_resourcesLoading.count(hash)) {
		...
		m_taskQueue->EnqueueTask([this, path, hash]() {
			...
			if (resource) {
				// This may cause the first buffer to be loaded twice and occupy two slots in the cache.
				// This may not be a problem though, as only the first entry will get touched and the second
				// entry will get removed from the cache when needed.

				// Immediately start loading the first buffer.
				RequestBuffer(hash, 0, NULL);
			}
		});
	}
...
enum BufferStatus ResourceCache::RequestBuffer(uint32_t resourceHash, unsigned int bufferIndex, ALuint *buffer_out)
{
	if (m_resourcesLoading[resourceHash]) {
		return BufferStatusLoadingHeaders;
	}
	...
	uint64_t bestSlotFrame = UINT64_MAX;
	int bestSlot = -1;
	int emptySlot = -1;
	...
	for (int i = 0; i < SOUNDENGINE_BUFFER_POOL_SIZE; i++) {
		if (m_cacheEntries[i].resourceHash == resourceHash && m_cacheEntries[i].bufferIndex == bufferIndex) {
			...
			else if (m_cacheEntries[i].lastRequestedFrame < bestSlotFrame) {
				bestSlotFrame = m_cacheEntries[i].lastRequestedFrame;
				bestSlot = i;
			}
		}
	}

	if (emptySlot == -1 && bestSlotFrame >= m_currentFrame - 1) {
	...
	m_cacheEntries[slot].resourceHash = resourceHash;
