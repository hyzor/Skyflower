...
SoundResourceWAV *SoundResourceWAV::Load(SoundEngineImpl *engine, const std::string &name)
{
	std::fstream file;
	...
	else
	{
		...
		resource = new SoundResourceWAV(engine, name, data, (size_t)dataHeader.size, info);
...
SoundResourceWAV::SoundResourceWAV(SoundEngineImpl *engine, const std::string &name, const char *data, size_t size, const SoundResourceInfo &info) :
	SoundResource(name, info, false)
{
	m_engine = engine;
	m_buffer = m_engine->GetBuffer();
...
SoundResourceWAV::~SoundResourceWAV()
{
	if (m_isStreaming)
		...
		m_engine->ReleaseBuffer(m_buffer);
}
