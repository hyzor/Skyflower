...
float4 main(VertexOut pIn) : SV_TARGET
{
	...
	float2 velocity = -SMAA_DECODE_VELOCITY(SMAASamplePoint(velocityTex, pIn.texCoord).rg);
		...
		// Fetch current pixel:
		float4 current = SMAASamplePoint(currentColorTex, pIn.texCoord);

		// Reproject current coordinates and fetch previous pixel:
		float4 previous = SMAASamplePoint(previousColorTex, pIn.texCoord + velocity);
	...
	float4 current = SMAASamplePoint(currentColorTex, pIn.texCoord);
		float4 previous = SMAASamplePoint(previousColorTex, pIn.texCoord);
