...
AnimEvaluatorSorted::AnimEvaluatorSorted(const aiAnimation* anim)
{
	mLastTime = 0.0f;
...
float AnimEvaluatorSorted::GetDuration(UINT frameStart, UINT frameEnd)
{
...
UINT AnimEvaluatorSorted::GetFrameIndexAt(float time)
{
...
UINT AnimEvaluatorSorted::GetFrameIndexAt(float time, UINT frameStart, UINT frameEnd)
{
	UINT nrOfFrames = (frameEnd - frameStart) + 1;
	float framesDuration = (float)nrOfFrames * (mDuration / static_cast<float>(Transforms.size()));
...
UINT AnimEvaluatorSorted::GetFrameIndexAt(float time, UINT frameStart, UINT frameEnd, bool playAnimForward)
{
	UINT nrOfFrames = (frameEnd - frameStart) + 1;
	float framesDuration = (float)nrOfFrames * (mDuration / static_cast<float>(Transforms.size()));
...
void AnimEvaluatorSorted::Evaluate(float pTime, std::map<std::string, SkinData::Bone*>& bones)
{
...
SkinnedDataSorted::~SkinnedDataSorted()
{
	CurrentAnimIndex = -1;
	Animations.clear();
	SafeDelete(Skeleton);
...
bool SkinnedDataSorted::SetAnimation(const std::string& name)
{
...
bool SkinnedDataSorted::SetAnimIndex(UINT animIndex)
{
...
void SkinnedDataSorted::Calculate(float time)
{
...
void SkinnedDataSorted::UpdateTransforms(SkinData::Bone* node)
{
...
void SkinnedDataSorted::CalcBoneMatrices()
{
...
UINT SkinnedDataSorted::GetAnimationIndex(const std::string& name)
{
	std::map<std::string, UINT>::iterator itr = AnimationNameToId.find(name);
	UINT animIndex = 0;
...
float SkinnedDataSorted::GetAnimationDuration(UINT animationIndex, UINT frameStart, UINT frameEnd)
{
